...
bot.on('message', async (msg) => {
  ...
  if (text === '/start') {
    await bot.sendMessage(chatId, 'Хочу ваши данные', {
      reply_markup: {
        inline_keyboard: [[{ text: 'Заполнить форму', web_app: { url: webAppUrl + '/form' } }], [{ text: 'В разработке...', web_app: { url: webAppUrl } }]],
      },
    })

    await bot.sendMessage(chatId, '...', {
      reply_markup: {
        keyboard: [[{ text: 'Заполнить форму', web_app: { url: webAppUrl + '/form' } }], [{ text: 'Нажать на кнопку' }], [{ text: 'Просто есть' }]],
      },
    })
  }
